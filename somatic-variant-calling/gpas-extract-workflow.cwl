class: Workflow
cwlVersion: v1.0
id: gpas-extract-workflow
requirements:
  - class: SubworkflowFeatureRequirement
  - class: MultipleInputFeatureRequirement
doc: |
  Extract and stage input files for GPAS somatic variant calling workflow.
...
outputs:
  ...
  known_indel_with_index:
    type: File
    outputSource: stage/indel_with_index
...
steps:
  normal_download:
    run: ../tools/util/bio_client_download.cwl
    in:
  ...
  known_indel_download:
    run: ../tools/util/bio_client_download.cwl
    in:
      config_file: bioclient_config
      download_handle: known_indel_gdc_id
    out: [output]

  known_indel_index_download:
    run: ../tools/util/bio_client_download.cwl
    in:
      config_file: bioclient_config
      download_handle: known_indel_index_gdc_id
    out: [output]
  ...
  stage:
    run: ./subworkflows/gpas_stage_workflow.cwl
    in:
      ...
      indel: known_indel_download/output
      indel_index: known_indel_index_download/output
    ...
    out: [normal_with_index, tumor_with_index, reference_with_index, indel_with_index, dbsnp_with_index, pon_with_index, cosmic_with_index]
